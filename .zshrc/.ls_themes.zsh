...
set_ls_theme() {
    # 1. Variables locales para evitar contaminación del shell [cite: 7]
    local theme_neon theme_nordic theme_gruvbox theme_classic
    local theme_solarized theme_dracula theme_catppuccin
    ...
    # 3. Aplicación del tema en LS_COLORS 
    case "$selected_theme" in
        "neon") export LS_COLORS="$theme_neon" ;;
        "nordic") export LS_COLORS="$theme_nordic" ;;
        "gruvbox") export LS_COLORS="$theme_gruvbox" ;;
        "classic") export LS_COLORS="$theme_classic" ;;
        "solarized") export LS_COLORS="$theme_solarized" ;;
        "dracula") export LS_COLORS="$theme_dracula" ;;
        "catppuccin") export LS_COLORS="$theme_catppuccin" ;;
        *) 
...
# 4. Sincronización con Starship (Arquitectura Limpia) ---
    local target="$HOME/.config/starship.toml"
    
    # El modificador :A resuelve symlinks y rutas relativas a absoluta instantáneamente
    target=$target:A

if [[ -f "$target" ]]; then
        if sed -i '' "s/^palette = .*/palette = \"$selected_theme\"/" "$target"; then
            # Línea silenciada para un inicio de terminal limpio
            # echo "Starship: [$selected_theme] (Path: $target)"
            : # No hace nada (placeholder de shell)
        fi
    fi
}

# 5. Autocompletado del sistema 
